...
// Put General de Vaccine
func UpdateVaccineGeneral(ctx *gin.Context) {
	// Se obtiene los datos del cuerpo de la peticion
	var updatedVaccine models.Vaccine
	//Si hubo un error
	if err := ctx.ShouldBindJSON(&updatedVaccine); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Error al procesar los datos de las Vaccines"})
		return
	}
	//Se actualiza el Vaccine
	updatedVaccine, err := services.UpdateVaccineGeneralService(updatedVaccine)
	//En caso de algun error
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Error al actualizar las Vaccines"})
		return
	}
	// Devuelve el mensaje de confirmación
	ctx.JSON(http.StatusCreated, "Vaccines actualizados")
}
