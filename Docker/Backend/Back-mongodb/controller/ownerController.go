...
func UpdateOwner(ctx *gin.Context) {
	...
	//En caso de algun error
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Error al actualizar al Dueños"})
		return
	}
	// Devuelve el mensaje de confirmación
...
// Put General de Owner
func UpdateOwnerGeneral(ctx *gin.Context) {
	// Se obtiene los datos del cuerpo de la peticion
	var updatedOwner models.Owner
	//Si hubo un error
	if err := ctx.ShouldBindJSON(&updatedOwner); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Error al procesar los datos de los Dueños"})
		return
	}
	//Se actualiza el Owner
	updatedOwner, err := services.UpdateOwnerGeneralService(updatedOwner)
	//En caso de algun error
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Error al actualizar los Dueños"})
		return
	}
	// Devuelve el mensaje de confirmación
	ctx.JSON(http.StatusCreated, "Dueños actualizados")
}
